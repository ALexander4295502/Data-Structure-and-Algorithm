...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	{
		Decreaser<T> arraytmp = array[fpos];
//		System.out.println("fpos= "+fpos);
//		System.out.println("spos= "+spos);
//		System.out.println("fpos loc = "+array[fpos].loc);
//		System.out.println("spos loc = "+array[spos].loc);
//		T tmp=array[fpos].getValue();
//		array[fpos].thing = array[spos].thing;
//		array[spos].thing = tmp;
		int fposloc = array[fpos].loc;
		int sposloc = array[spos].loc;
		array[fpos] = array[spos];
		array[spos] = arraytmp;
		array[fpos].loc = fposloc;
		array[spos].loc = sposloc;
//		System.out.println("fpos= "+fpos);
//		System.out.println("spos= "+spos);
//		System.out.println("fpos loc = "+array[fpos].loc);
//		System.out.println("spos loc = "+array[spos].loc);
	...
	public Decreaser<T> insert(T thing) {
		...
		//System.out.println("the loc is: "+this.array[size].loc);
		ticker.tick();
		int current = size;
		if(size>1){
//			System.out.println("Current: ");
//			System.out.println(array[current].getValue());
//			System.out.println("Parent: ");
//			System.out.println(array[parent(current)].getValue());	
		}
		
//		if(size>1){
//			
//			while(current > 1 && (array[current].getValue().compareTo(array[parent(current)].getValue())<0))
//			{
//				
//				//System.out.println("SSSWAP: ");
...
//		array[fpos].thing = array[spos].thing;
//		array[spos].thing = tmp;
		int sizeposloc = array[1].loc;
		array[1] = array[size];
		array[1].loc = sizeposloc;
		ticker.tick();
//		System.out.println("fpos= "+fpos);
//		System.out.println("spos= "+spos);
//		System.out.println("fpos loc = "+array[fpos].loc);
//		System.out.println("spos loc = "+array[spos].loc);
	...
	private void heapify(int where) {
		...
		//  FIXME
		//
		//System.out.println("where = "+where+"  num = "+array[where].getValue());
		if(!(isLeaf(where)) && size >1)  //to do this if where is not leaf
		{ 
			//System.out.println("heap in this?");
			if(where == size/2 && size%2 == 0){  //!!!!!!!!!!!!!!!!!!!!!!! where doesnt have right child!!!!!!!!!!!!!
				if(array[where].getValue().compareTo(array[leftchild(where)].getValue()) > 0)
				{
					//System.out.println("where == size/2");
					swap(where,leftchild(where));
					heapify(leftchild(where));
				}
					
			}else{
			//System.out.println("I am in heap not leaf");
			if(array[where].getValue().compareTo(array[leftchild(where)].getValue()) > 0  || array[where].getValue().compareTo(array[rightchild(where)].getValue()) > 0 )
			{
				if(array[leftchild(where)].getValue().compareTo(array[rightchild(where)].getValue()) < 0)
				{
					//System.out.println("leftchild = "+array[leftchild(where)].getValue()+"rightchild = "+array[rightchild(where)].getValue());
					swap(where,leftchild(where));
					heapify(leftchild(where));
				}else
				{
					//System.out.println("leftchild = "+array[leftchild(where)].getValue()+"rightchild = "+array[rightchild(where)].getValue());
					swap(where,rightchild(where));
					heapify(rightchild(where));
				}
			}
			}
		}
